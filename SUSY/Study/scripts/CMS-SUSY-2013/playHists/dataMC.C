...
vector<TH1D*> dataMC::getHists( bool MuAddOrNot, TString HTBins, int whichpart, int rebin, TString xAxisName, TString yAxisName, double xAxisRange1, double xAxisRange2, int dataMC, TString whichplot, bool separateSample, TString singleMCsample, double lowy, double highy, int OneDTwoD, int startNJet, int nJets, TString MuonNumber, TString FolderLabel ){
...
void dataMC::drawHists( bool MuAddOrNot, TString HTBins, int whichpart, int rebin, TString xAxisName, TString yAxisName, double xAxisRange1, double xAxisRange2, TString whichplot, TLegend * len, double lowy, double highy, int OneDTwoD, int startNJet, int nJets, TString MuonNumber, TString FolderLabel ){
...
//      datah->GetYaxis()->SetTitle("t#bar{t}/t#bar{t}_MasiveBin");
  ...
  pad1->SetLogy(0);
  pad1->RedrawAxis();
  c1->SaveAs( Form( "%s"+whichplot+"_%s%s_%s%s%iTo%i%s.%s", stack.Data(), sele.Data(), HTBins.Data(), MuonNumber.Data(), FolderLabel.Data(), startNJet-1, nJets+startNJet-2, borj.Data(), epspng_.Data() ) );
  pad1->SetLogy();
  pad1->RedrawAxis();
  c1->SaveAs( Form( "%s"+whichplot+"_%s%s_%s%s%iTo%i%s_log.%s",  stack.Data(), sele.Data(), HTBins.Data(), MuonNumber.Data(), FolderLabel.Data(), startNJet-1, nJets+startNJet-2, borj.Data(), epspng_.Data() ) );
...
TH1D* dataMC::getRatioHists( bool MuAddOrNot, TString HTBins, int whichpart, int rebin, TString xAxisName, TString yAxisName, double xAxisRange1, double xAxisRange2, TString whichplot, TLegend * len, double lowy, double highy, int OneDTwoD, int startNJet, int nJets, TString MuonNumber, TString FolderLabel ){
  vector<TString> vhnames;
  vector<TH1D*> vh;
...
void dataMC::getRatioOnRatio( TString HTBins, int startNJet, int nJets, TString MuonNumber, TString FolderLabel, TString xname, TString whichplot, int rebin, double x1, double x2 ){
...
void dataMC::getRatioOnRatioResults( TString HTBins, int startNJet, int nJets, TString MuonNumber, TString FolderLabel ){
  int rebin=50;
...
void dataMC::getResults( TString HTBins, TString selection, int startNJet, int nJets, TString MuonNumber, TString FolderLabel ){
  TLegend *len=new TLegend( 0.70, 0.60, 0.88, 0.88 );
